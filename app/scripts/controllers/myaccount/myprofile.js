...
/**
 * @ngdoc function
 * @name dataduduR3App.controller:MyProfileCtrl
 * @description
 * # MyProfileCtrl
...
.controller('MyProfileCtrl', function($scope, $q, $filter, $window, $timeout, $log, $http, $route,
  ...
  $scope.saveAccount = function(form){
    ...
    if($scope.validatedForm.$invalid) return;
    ...
    if(form.isChangePassword) {
      payload.new_password = form.newPassword1;
    }

    account.updateAccount(payload)
      .$promise
      .then(function(resp){
  ...
  $scope.openModalUploadAvatar = function(){
    $uibModal.open({
        templateUrl: 'views/shared/modalUploadAvatar.html',
        controller: 'UploadAvatarCtrl',
        backdrop: false,
        resolve: {
          account_key: function() {
            return $scope.form.account_key;
          }
        }
      })
      .result
  ...
  $scope.cityOptions = {
    "中国": {
      ...
      "重庆": ["万州", "涪陵", "渝中", "大渡口", "江北", "沙坪坝", "九龙坡", "南岸", "北碚", "万盛", "双桥", "渝北", "巴南", "长寿", "綦江", "潼南", "铜梁", "大足", "荣昌", "璧山", "梁平", "城口", "丰都", "垫江", "武隆", "忠县", "开县", "云阳", "奉节", "巫山", "巫溪", "黔江", "石柱", "秀山", "酉阳", "彭水", "江津", "合川", "永川", "南川"],
